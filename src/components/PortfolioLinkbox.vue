          ...
          >
            <div class="flex flex-col items-center justify-center w-full">
              <div class="bgfilterblur p-1 rounded-[2.5rem]">
                ...
                <button
                  @click="isEditPortfolioData = !isEditPortfolioData"
                  class="cursor-pointer pointer-events-auto rounded-[4rem] outline-none mr-2 !p-1.5 text-black"
                  :class="
                    isEditPortfolioData
                      ...
                      : ''
                  "
                  :color="isEditPortfolioData ? '#FFFFFF' : '#676B5F'"
                >
                  <svg
                    xmlns="http://www.w3.org/2000/svg"
                    fill="none"
...
const updatePortfolioThumbnail = async () => {
  try {
    await store
      .dispatch("updateThumbnailLinkImage", data.value)
      .then(({ data }) => {
        store.commit("notify", {
          type: "success",
          message: "Saved",
...
const deleteLink = async () => {
  ...
  try {
    if (res) {
      await store.dispatch("deleteLink", link.value.id).then(({ data }) => {
        store.commit("notify", {
          type: "success",
          message: "Saved",
        });
