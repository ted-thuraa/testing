<template>
    ...
    >
      ...
      <div class="relative flex-1 flex overflow-y-hidden overflow-x-hidden">
        ...
        <div class="relative flex-1 overflow-y-auto md:overflow-hidden">
          <div
            class="w-full flex h-screen overflow-y-auto flex-1 flex-col px-6 py-12 lg:px-8"
          >
            <section class="sm:mx-auto sm:w-full sm:max-w-xl">
              <div class="">
                <div class="mb-3">
                  <img
                    class="mx-auto rounded-full w-[7rem] h-[7rem] lg:h-[11rem] lg:w-[11rem] ring-4 object-cover"
                    :class="theme.profilePic_ring"
                    :src="store.state.guestpagedata.data.page.linkbioimage"
                    decoding="async"
                    :alt="store.state.guestpagedata.data.page.owner_fullname"
                  />
                </div>
                <div>
                  <h2
                    class="mt-1 text-center text-2xl font-bold leading-9 tracking-tight"
                    :class="theme.htext"
                  >
                    {{ store.state.guestpagedata.data.page.owner_fullname }}
                  </h2>
                </div>
                ...
                <div class="flex flex-row gap-x-2 justify-center items-center">
                  ...
                  <div class="flex flex-col lg:flex-row gap-1 lg:gap-4">
                    <h2 class="flex gap-2 items-center" :class="theme.ptext">
                      <svg
                        xmlns="http://www.w3.org/2000/svg"
                        viewBox="0 0 24 24"
                        fill="currentColor"
                      ...
                      <span class="text-[0.9rem] font-medium leading-6">{{
                        store.state.guestpagedata.data.page.location
                      }}</span>
                    </h2>
                  </div>
                  <!-- <h2 class="">
                    <span
                      ...
                      :class="theme.ptext"
                      >@{{ store.state.guestpagedata.data.page.linkname }}</span
                    >
                  </h2> -->
                </div>
              </div>
              <div class="sm:mx-auto sm:max-w-lg">
                <div
                  class="w-full my-6 flex items-center lg:text-lg leading-relaxed -space-y-4"
                >
                  <p
                    class="text-[0.9rem] text-center font-medium leading-6 md:text-lg pb-4"
                    :class="theme.ptext"
                  >
                    {{ store.state.guestpagedata.data.page.linkbio }}
                  </p>
                </div>
                <div class="mx-6 h-[1px] bg-gray-500 md:hidden"></div>
              </div>
            </section>

            <!-- <section class="sm:mx-auto sm:w-full sm:max-w-3xl">
              <div
                class="w-full space-y- p-4 grid grid-cols-2 gap-x-4 lg:py-16 lg:px-20 lg:grid lg:grid-cols-2 md:gap-4"
              >
                <div
                  v-for="link in model.allLinks"
                  :key="link"
                  class="m-0 p-0"
                  :class="[
                    {
                    ...
                    {
                      'lg:col-span-2': link.layout === 'Verticalbar',
                    },
                    {
                      'lg:col-span-2': link.layout === 'Horizontalbar',
                    },
                    {
                      'lg:col-span-2': link.layout === 'Block',
                    },
                    {
                      'lg:col-span-2': !link.layout,
                    },
                  ]"
                >
                  <component
              ...
              <div class="block lg:hidden">
                ...
                >
                  <li v-for="icon in model.sociallinks" :key="icon">
                    <a
                      class=""
                      href=""
                      target="_blank"
            ...
            <section class="sm:mx-auto sm:w-full sm:max-w-3xl">
              <div
                class="w-full space-y- grid grid-cols-2 gap-x-4 lg:py-16 lg:px-20 lg:grid lg:grid-cols-2 md:gap-4"
              >
                <div
                  v-for="link in model.allLinks"
                  :key="link"
  ...
  <!-- <div
    v-if="!pageLoading"
    class="bg-white flex h-screen overflow-y-auto flex-1 flex-col px-6 py-12 lg:px-8"
  >
...
const { model, theme, pageLoading } = toRefs(props);
...
// onMounted(() => {
//   const links_ordered = computed(() =>
//     store.state.guestpagedata.data.links
//       .slice()
//       .sort((a, b) => a.order - b.order)
//   );
//   allLinks.value = links_ordered.value;
//   sociallinks.value = store.state.guestpagedata.data.SocialIcons;
