<template>
  <div class="w-full rounded-3xl">
    <div>
      <div id="MainLinkBoxSection" class="relative px-8">
        <div class="p-[0.375rem]">
          <div class="flex w-full">
            <div class="p-1 rounded-[2.5rem] flex flex-wrap">
              <button
                ...
                "
                class="cursor-pointer text-gray-700 pointer-events-auto rounded-[4rem] outline-none mr-2 !p-2"
              >
                <div class="w-6 h-6" v-html="getSocialIcon(icon.name)"></div>
              </button>
            </div>
          </div>
          <div
            v-if="editUrl"
            class="relative flex flex-row items-center w-full bg-[#DFE2D9] py-1"
          >
            <div class="w-[100%] text-center text-sm font-bold text-gray-950">
              close
            </div>
            <svg
              @click="editUrl = false"
              ...
              stroke-width="1.5"
              stroke="currentColor"
              class="cursor-pointer w-5 h-5 mr-2"
            >
              <path
          ...
          <div
            ...
            :class="[
              {
                'max-h-[160px] transition-all duration-300 ease-in mt-1':
                  editUrl,
              },
              {
                'max-h-0 transition-all duration-200 ease-out': !editUrl,
              },
            ]"
          >
            <div v-for="input in iconinputs" :key="input">
              <div v-if="toEdit">
                <div v-if="input.name === toEdit.name" class="relative">
                  <div class="relative flex flex-row w-full font-medium mt-6">
                    <input
                      type="text"
                      v-model="toEdit.url"
                      id="input-group-1"
                      class="w-full bg-gray-50 border-gray-200 focus:bg-gray-50 focus:border-opacity-0 focus:border-gray-100 text-gray-900 text-sm rounded-lg focus:ring-blue-50 block pl-6 p-2.5"
                      :placeholder="`Enter ${toEdit.name} url`"
                    />
                    <div
                    ...
                    >
                      <svg
                        ...
                        viewBox="0 0 24 24"
                        fill="currentColor"
                        class="pl-1 w-4 h-4 text-gray-500"
                      >
                        <path
                    ...
                    <button @click="deleteIcon" class="ml-4 text text-red-500">
                      <svg
                        xmlns="http://www.w3.org/2000/svg"
                        fill="none"
                        viewBox="0 0 24 24"
                  ...
                  <div
                    class="mt-2 mb-4 w-full flex flex-row items-center justify-center"
                  >
                    <button @click="updateicon" class="text-gray-200">
                      <!-- Animation Snippet -->
                      <div
                        class="group [transform:translateZ(0)] px-40 py-3 rounded-[1.5rem] overflow-hidden bg-gray-200 relative before:absolute before:bg-purple-700 before:top-1/2 before:left-1/2 before:h-8 before:w-8 before:-translate-y-1/2 before:-translate-x-1/2 before:rounded-full before:scale-[0] before:opacity-0 hover:before:scale-[12] hover:before:opacity-100 before:transition before:ease-in-out before:duration-500"
                      >
                        <span
                          class="relative z-0 text-black group-hover:text-gray-200 transition ease-in-out duration-500"
                          >Save</span
                        >
                      </div>
                    </button>
...
const { socialicons } = toRefs(props);
let socialiconurl = ref("");
let active = ref(false);
let editUrl = ref(false);
let toEdit = ref(null);
let errors = ref(null);
...
const deleteIcon = async () => {
  ...
  try {
    if (res) {
      await store.dispatch("deleteSocialIcon", {
        id: toEdit.value.id,
      });
      //await store.dispatch("getAllSocialIcons");
    }
