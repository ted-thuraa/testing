<template>
  <!-- eslint-disable -->
  <div class="text-gray-900 mb-4 flex justify-between items-center">
    <h2
      class="font-semibold !"
    ...
    <button class="text-gray-800" tabindex="0">
      <svg
  ...
  <div class="text-gray-900 flex flex-wrap gap-2">
    <button
      class="p-1 flex items-center bg-gray-200 rounded-full !w-[40px] !h-[40px] aspect-square border-0 flex-none"
    >
      <svg
        xmlns="http://www.w3.org/2000/svg"
        fill="none"
  ...
  <div
    class="inline-flex text-gray-900 mt-4 p-4 rounded-lg items-center hover:bg-base-200 duration-200 cursor-pointer"
  >
    <svg
      xmlns="http://www.w3.org/2000/svg"
      viewBox="0 0 24 24"
    ...
    <div class="px-1">
      <input
        type="text"
        class="truncate outline-none bg-gray-50 rounded-lg border-white"
        v-model="cpyLink"
        readonly
      />
    </div>
    <button v-if="isCopied" class="font-semibold ml-auto text-green-600">
      Copied
    </button>
    <button v-else @click="copyLink" class="font-semibold ml-auto">Copy</button>
  </div>
</template>
...
const { pageusername } = toRefs(props);

onMounted(() => {
  username.value = pageusername.value;
  cpyLink.value = `https://creatorpa.ge/${username.value}`;
});

function copyLink() {
  console.log(cpyLink.value);
  try {
    navigator.clipboard.writeText(cpyLink.value);
    console.log("copied");
    isCopied.value = true;
    setTimeout(() => {
