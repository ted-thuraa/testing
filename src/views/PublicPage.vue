<template>
  <!-- eslint-disable -->
  <div class="w-full h-screen">
    <Template1
      v-if="page_layout == 'PanoramaPassage'"
      :model="model"
      :theme="theme"
      :pageLoading="pageLoading"
    />
    <Template2
      v-else
      :model="model"
      :theme="theme"
      :pageLoading="pageLoading"
    />
  </div>

  <!-- <div
    v-if="theme"
    class="max-w-[390px] md:max-w-full w-full relative flex flex-col overflow-x-hidden md:overflow-y-hidden h-screen"
    :style="{ fontFamily: font }"
    :class="theme.pageBg"
  >
    <div class="w-full mt-4 md:mt-0">
      <div class="fixed top-0 right-0 left-0 z-40 md:px-6">
        <div class="relative w-full mt-4">
          <div class="flex justify-end">
            <Popover v-slot="{ open }" class="">
              <PopoverButton
                :class="open ? '' : 'text-opacity-90'"
                class="group inline-flex items-center justify-center p-2 md:py-2.5 md:px-5 mr-2 mb-2 text-sm font-medium text-white focus:outline-none bg-black/40 rounded-full border border-gray-200 focus:z-10 focus:ring-4 focus:ring-gray-200"
              >
                <svg
                  xmlns="http://www.w3.org/2000/svg"
                  fill="none"
                  viewBox="0 0 24 24"
                  stroke-width="1.5"
                  stroke="currentColor"
                  class="md:mr-2 w-5 h-5"
                >
                  <path
                    stroke-linecap="round"
                    stroke-linejoin="round"
                    d="M7.217 10.907a2.25 2.25 0 100 2.186m0-2.186c.18.324.283.696.283 1.093s-.103.77-.283 1.093m0-2.186l9.566-5.314m-9.566 7.5l9.566 5.314m0 0a2.25 2.25 0 103.935 2.186 2.25 2.25 0 00-3.935-2.186zm0-12.814a2.25 2.25 0 103.933-2.185 2.25 2.25 0 00-3.933 2.185z"
                  />
                </svg>
                <span class="hidden md:block">Share page</span>
              </PopoverButton>

              <transition
                enter-active-class="transition duration-200 ease-out"
                enter-from-class="translate-y-1 opacity-0"
                enter-to-class="translate-y-0 opacity-100"
                leave-active-class="transition duration-150 ease-in"
                leave-from-class="translate-y-0 opacity-100"
                leave-to-class="translate-y-1 opacity-0"
              >
                <PopoverPanel
                  class="absolute max-w-sm right-0 left-0 mx-auto z-10 mt-3 transform px-4 sm:px-0"
                >
                  <div
                    class="p-6 bg-white w-full overflow-hidden rounded-[1rem] shadow-lg ring-1 ring-black ring-opacity-5"
                  >
                    <SocialShareBtns :pageusername="username" />
                  </div>
                </PopoverPanel>
              </transition>
            </Popover>
          </div>
        </div>
      </div>
    </div>

    <div class="relative flex-1 flex overflow-y-hidden overflow-x-hidden">
      <div
        class="absolute top-0 right-0 bottom-0 left-0 w-full h-full bg-transparent bg-cover bg-fixed"
        :class="theme.themecustombgImg"
      >
        <div
          v-if="theme.hasusercustombgImg"
          class="w-[200%] -translate-x-1/4 opacity-[0.7] blur-[30px] inline-block bg-transparent"
        >
          <img
            loading="lazy"
            :src="image"
            alt=""
            class="h-auto w-[100%] block m-auto"
          />
        </div>
        <div
          v-if="theme.bgImgWithOverlay"
          class="absolute top-0 bottom-0 left-0 right-0"
          :class="theme.bgImgOverlay"
        ></div>
      </div>
      <div class="relative flex-1 overflow-y-auto md:overflow-hidden">
        <div class="w-full min-h-screen flex flex-col lg:flex-row">
          <section
            class="shrink-0 space-y-4 px-4 lg:space-y-8 lg:w-[400px] xl:w-[550px] lg:px-16 lg:py-12 lg:pr-0 xl:pr-16"
          >
            <div
              class="relative h-min pb-2 md:pb-10 w-full rounded-[1.5rem]"
              :class="theme.profileAreaBg"
            >
              <div
                class="relative block overflow-visible opacity-100 pt-6 px-6 pb-2 w-full rounded-xl"
              >
                <div
                  class="flex items-centre flex-row flex-nowrap lg:flex-col h-min gap-8 md:pl-0"
                >
                  <div class="relative flex flex-none overflow-visible lg:mb-2">
                    <img
                      loading="lazy"
                      :src="image"
                      decoding="async"
                      alt=""
                      class="h-[7rem] w-[7rem] lg:w-44 lg:h-44 rounded-full ring-4 object-cover"
                      :class="theme.profilePic_ring"
                    />
                  </div>
                  <div
                    class="ml-2.5 relative h-auto flex flex-col items-center lg:items-start"
                  >
                    <div class="space-y-1 flex-1">
                      <h1
                        class="text-2xl font-extrabold lg:font-extrabold lg:text-4xl lg:mb-3 leading-9 tracking-tight"
                        :class="theme.htext"
                      >
                        {{ fullname }}
                      </h1>
                      <h2
                        class="inline-flex gap-2 items-center"
                        :class="theme.ptext"
                      >
                        <svg
                          xmlns="http://www.w3.org/2000/svg"
                          viewBox="0 0 24 24"
                          fill="currentColor"
                          class="w-5 h-5"
                        >
                          <path
                            fill-rule="evenodd"
                            d="M7.5 5.25a3 3 0 013-3h3a3 3 0 013 3v.205c.933.085 1.857.197 2.774.334 1.454.218 2.476 1.483 2.476 2.917v3.033c0 1.211-.734 2.352-1.936 2.752A24.726 24.726 0 0112 15.75c-2.73 0-5.357-.442-7.814-1.259-1.202-.4-1.936-1.541-1.936-2.752V8.706c0-1.434 1.022-2.7 2.476-2.917A48.814 48.814 0 017.5 5.455V5.25zm7.5 0v.09a49.488 49.488 0 00-6 0v-.09a1.5 1.5 0 011.5-1.5h3a1.5 1.5 0 011.5 1.5zm-3 8.25a.75.75 0 100-1.5.75.75 0 000 1.5z"
                            clip-rule="evenodd"
                          />
                          <path
                            d="M3 18.4v-2.796a4.3 4.3 0 00.713.31A26.226 26.226 0 0012 17.25c2.892 0 5.68-.468 8.287-1.335.252-.084.49-.189.713-.311V18.4c0 1.452-1.047 2.728-2.523 2.923-2.12.282-4.282.427-6.477.427a49.19 49.19 0 01-6.477-.427C4.047 21.128 3 19.852 3 18.4z"
                          />
                        </svg>

                        <span
                          class="text-[0.9rem] font-medium leading-6"
                          :class="theme.ptext"
                          >{{ creator_category }}</span
                        >
                      </h2>
                      <div class="flex flex-col lg:flex-row gap-1 lg:gap-4">
                        <h2
                          class="flex gap-2 items-center"
                          :class="theme.ptext"
                        >
                          <svg
                            xmlns="http://www.w3.org/2000/svg"
                            viewBox="0 0 24 24"
                            fill="currentColor"
                            class="w-5 h-5"
                          >
                            <path
                              fill-rule="evenodd"
                              d="M11.54 22.351l.07.04.028.016a.76.76 0 00.723 0l.028-.015.071-.041a16.975 16.975 0 001.144-.742 19.58 19.58 0 002.683-2.282c1.944-1.99 3.963-4.98 3.963-8.827a8.25 8.25 0 00-16.5 0c0 3.846 2.02 6.837 3.963 8.827a19.58 19.58 0 002.682 2.282 16.975 16.975 0 001.145.742zM12 13.5a3 3 0 100-6 3 3 0 000 6z"
                              clip-rule="evenodd"
                            />
                          </svg>

                          <span class="text-[0.9rem] font-medium leading-6">{{
                            location
                          }}</span>
                        </h2>
                        <div>
                          <div class="text-base flex gap-1 items-center"></div>
                        </div>
                      </div>

                      <h2 class="">
                        <span
                          class="text-[0.9rem] font-medium leading-6"
                          :class="theme.ptext"
                          >@{{ username }}</span
                        >
                      </h2>
                    </div>

                    <div></div>
                  </div>
                </div>
              </div>
              <div
                class="w-full px-2 my-5 flex items-center lg:text-lg leading-relaxed -space-y-4"
              >
                <p
                  class="max-h-[120px] overflow-hidden text-[0.9rem] font-medium leading-6 md:text-lg pb-4"
                  :class="theme.ptext"
                >
                  {{ bio }}
                </p>
              </div>
              <div
                class="mx-6 h-[1px] md:hidden"
                :class="theme.profileSec_divider"
              ></div>
              <div class="hidden lg:block px-2">
                <ul
                  class="flex justify-center items-center gap-2 lg:gap-4 lg:justify-start flex-wrap max-lg:p-6"
                >
                  <li v-for="icon in sociallinks">
                    <a
                      class=""
                      href=""
                      target="_blank"
                      aria-label="Open link to Twitter"
                      ><span
                        class="transform duration-200 group-hover:scale-110 fill-base-content"
                      >
                        <div
                          class="w-6 h-6"
                          :class="theme.ptext"
                          v-html="getIconItem(icon.name)"
                        ></div> </span
                    ></a>
                  </li>
                </ul>
              </div>
            </div>
          </section>

          <section
            class="mx-auto w-full lg:h-screen overflow-x-hidden lg:overflow-y-scroll"
          >
            <div
              class="w-full space-y- p-4 grid grid-cols-2 gap-x-4 lg:py-16 lg:px-20 lg:grid lg:grid-cols-2 md:gap-4"
            >
              <div
                v-for="link in allLinks"
                :key="link"
                class="m-0 p-0"
                :class="[
                  {
                    '': link.layout === 'Cube',
                  },
                  {
                    'col-span-2': link.layout === 'Verticalbar',
                  },
                  {
                    'col-span-2': link.layout === 'Horizontalbar',
                  },
                  {
                    'col-span-2': link.layout === 'Block',
                  },
                  {
                    'col-span-2': !link.layout,
                  },
                  {
                    '!col-span-1': link.category === 'Text',
                  },
                ]"
              >
                <component
                  :is="getComponentItem(link.category)"
                  :link="link"
                  :theme="theme"
                  class=""
                />
              </div>
            </div>
            <div class="block lg:hidden">
              <ul
                class="flex justify-center items-center gap-2 lg:gap-4 lg:justify-start flex-wrap max-lg:p-6"
              >
                <li v-for="icon in sociallinks">
                  <a
                    class=""
                    href=""
                    target="_blank"
                    aria-label="Open link to Twitter"
                    ><span
                      class="transform duration-200 group-hover:scale-110 fill-base-content"
                    >
                      <div
                        class="w-6 h-6"
                        :class="theme.ptext"
                        v-html="getIconItem(icon.name)"
                      ></div> </span
                  ></a>
                </li>
              </ul>
            </div>
          </section>
        </div>
      </div>
    </div>
  </div>
  <SidePanelPop
    :isMoreInfoOpen="isMoreInfoOpen"
    @closeMoreInfoModal="closeMoreInfoModal"
  />
  <FormPanel :isFormOpen="isFormOpen" @closeFormOpen="closeFormOpen" /> -->
</template>

<script setup>
import { h } from "vue";
import { Tweet, Moment, Timeline } from "vue-tweet-embed";
import { Popover, PopoverButton, PopoverPanel } from "@headlessui/vue";
import {
  TransitionRoot,
  TransitionChild,
  Dialog,
  DialogPanel,
  DialogTitle,
} from "@headlessui/vue";
import SocialShareBtns from "../components/SocialShareBtns.vue";
import Template1 from "../components/PublicPageComponents/layout/Template1.vue";
import Template2 from "../components/PublicPageComponents/layout/Template2.vue";
import LinkItem from "../components/PublicPageComponents/Link.vue";
import StartupItem from "../components/PublicPageComponents/Startup.vue";
import ImageItem from "../components/PublicPageComponents/Image.vue";
import TextItem from "../components/PublicPageComponents/Text.vue";
import HeaderItem from "../components/PublicPageComponents/SectionHeader.vue";
import FormItem from "../components/PublicPageComponents/Form.vue";
import PortfolioItem from "../components/PublicPageComponents/Portfolio.vue";
import VideoItem from "../components/PublicPageComponents/Video.vue";
import StoreItem from "../components/PublicPageComponents/Store.vue";
import SidePanelPop from "../components/popovers/SidePanelPop.vue";
import FormPanel from "../components/popovers/FormPanelPhone.vue";
import DigitalProduct from "../components/PublicPageComponents/DigitalProductData.vue";
import MoreInfo from "../components/popovers/MoreInfo.vue";
import FormsModal from "../components/FormsModal.vue";
//import DefaultPage from "~/layouts/LinkPage/DefaultPage.vue";
//import IndiePage from "~/layouts/LinkPage/IndiePage.vue";
//import MarkettingPage from "~/layouts/LinkPage/MarkettingPage.vue";
//import ParcelPage from "~/layouts/LinkPage/ParcelPage.vue";
import { ref, watch, computed, onMounted, isProxy, toRaw } from "vue";
import { useStore } from "vuex";
import { useRouter, useRoute } from "vue-router";
import FormsModalVue from "../components/FormsModal.vue";

const store = useStore();

const router = useRouter();
const route = useRoute();

const linkitems = computed(() => store.state.linkitems);

let model = ref({
  fullname: "",
  username: "",
  bio: "",
  font: "",
  creator_category: "",
  location: "",
  image: "",
  allLinks: "",
  sociallinks: "",
});
let fullname = ref("");
//let usersname = ref("");
let username = ref("");
let bio = ref("");
let font = ref();
let creator_category = ref("");
let location = ref("");
let page_layout = ref("");
let theme_id = ref("");
let image = ref("");
let theme = ref(null);
let data = ref(null);
let errors = ref(null);
let colors = ref(null);
let allLinks = ref(null);
let sociallinks = ref(null);
let isMoreInfoOpen = ref(false);
let pageLoading = ref(true);
let currentInfoblog = ref(null);
let isFormOpen = ref(false);
let currentForm = ref(null);

console.log(route.params.username);

function closeMoreInfoModal() {
  isMoreInfoOpen.value = false;
}
function openMoreInfoModal() {
  isMoreInfoOpen.value = true;
}
function closeFormOpen() {
  isFormOpen.value = false;
}
function openFormOpen() {
  isFormOpen.value = true;
}

const getLinkTheme = (theme_id) => {
  colors.value.forEach((color) => {
    if (theme_id === color.id) {
      theme.value = color;
    }
  });
};

console.log(theme);

const them_id = computed(() => store.state.guestpagedata.themeId);

const getPageData = async (username) => {
  pageLoading.value = true;
  try {
    console.log(username.value);
    await store.dispatch("getPage", username.value).then(({ data }) => {
      console.log(data);
    });

    console.log(theme_id.value);
    getLinkTheme(them_id.value);
    pageLoading.value = false;
    store.dispatch("triggerAnalytics", username.value);
  } catch (error) {
    console.log(error);
    pageLoading.value = false;
    errors.value = error.response.data.errors;
  }
};

const gottYa = async (link_id) => {
  console.log(link_id);
  console.log(username.value);
  try {
    const res = await store.dispatch("gottYaClicks", link_id, username.value);
    console.log(res);
  } catch (error) {
    console.log(error);
  }
};

onMounted(() => {
  colors.value = store.state.user.colors;
  username.value = route.params.username;
  theme.value = null;
  getPageData(username);
  //getPageLinks(username);
});

console.log(them_id.value);

function setPageData() {
  console.log(store.state.guestpagedata.data);
  model.value.fullname = store.state.guestpagedata.data.page.owner_fullname;
  //usersname.value = store.state.guestpagedata.data.page.linkname;
  model.value.bio = store.state.guestpagedata.data.page.linkbio;
  model.value.creator_category =
    store.state.guestpagedata.data.page.creator_category;
  model.value.location = store.state.guestpagedata.data.page.location;
  model.value.font = store.state.guestpagedata.data.page.font;
  theme_id.value = store.state.guestpagedata.data.page.theme_id;
  page_layout.value = store.state.guestpagedata.data.page.page_layout;
  model.value.image = store.state.guestpagedata.data.page.linkbioimage;
  const links_ordered = computed(() => store.state.guestpagedata.data.links);
  model.value.allLinks = links_ordered.value;
  model.value.sociallinks = store.state.guestpagedata.data.SocialIcons;
  console.log(sociallinks.value);
}
function getComponentItem(category) {
  switch (category) {
    case "Link":
      return LinkItem;
    case "SaaS":
      return StartupItem;
    case "Image":
      return ImageItem;
    case "Text":
      return TextItem;
    case "Header":
      return HeaderItem;
    case "Form":
      return FormItem;
    case "Portfolio":
      return PortfolioItem;
    case "Video":
      return VideoItem;
    case "Map":
      return MapItem;
    case "Store":
      return StoreItem;
    case "DigitalProduct":
      return DigitalProduct;
    // Handle other categories here if needed
    default:
      return LinkItem; // Replace 'DefaultComponent' with a fallback component name
  }
}
function getIconItem(name) {
  switch (name) {
    case "X":
      return `
    <svg class="" viewBox="0 0 24 24" fill="currentColor"><path d="m.059.769 9.266 12.39L0 23.23h2.1l8.163-8.819 6.596 8.82H24l-9.788-13.087 8.68-9.376h-2.099L13.275 8.89 7.2.769H.06Zm3.086 1.545h3.28l14.488 19.371h-3.28L3.144 2.315Z"></path></svg>
        `;
    case "Instagram":
      return `
    <svg class="" viewBox="0 0 24 24" fill="currentColor"><path d="M21.938,7.71a7.329,7.329,0,0,0-.456-2.394,4.615,4.615,0,0,0-1.1-1.694,4.61,4.61,0,0,0-1.7-1.1,7.318,7.318,0,0,0-2.393-.456C15.185,2.012,14.817,2,12,2s-3.185.012-4.29.062a7.329,7.329,0,0,0-2.394.456,4.615,4.615,0,0,0-1.694,1.1,4.61,4.61,0,0,0-1.1,1.7A7.318,7.318,0,0,0,2.062,7.71C2.012,8.814,2,9.182,2,12s.012,3.186.062,4.29a7.329,7.329,0,0,0,.456,2.394,4.615,4.615,0,0,0,1.1,1.694,4.61,4.61,0,0,0,1.7,1.1,7.318,7.318,0,0,0,2.393.456c1.1.05,1.472.062,4.29.062s3.186-.012,4.29-.062a7.329,7.329,0,0,0,2.394-.456,4.9,4.9,0,0,0,2.8-2.8,7.318,7.318,0,0,0,.456-2.393c.05-1.1.062-1.472.062-4.29S21.988,8.814,21.938,7.71Zm-1,8.534a6.351,6.351,0,0,1-.388,2.077,3.9,3.9,0,0,1-2.228,2.229,6.363,6.363,0,0,1-2.078.388C15.159,20.988,14.8,21,12,21s-3.159-.012-4.244-.062a6.351,6.351,0,0,1-2.077-.388,3.627,3.627,0,0,1-1.35-.879,3.631,3.631,0,0,1-.879-1.349,6.363,6.363,0,0,1-.388-2.078C3.012,15.159,3,14.8,3,12s.012-3.159.062-4.244A6.351,6.351,0,0,1,3.45,5.679a3.627,3.627,0,0,1,.879-1.35A3.631,3.631,0,0,1,5.678,3.45a6.363,6.363,0,0,1,2.078-.388C8.842,3.012,9.205,3,12,3s3.158.012,4.244.062a6.351,6.351,0,0,1,2.077.388,3.627,3.627,0,0,1,1.35.879,3.631,3.631,0,0,1,.879,1.349,6.363,6.363,0,0,1,.388,2.078C20.988,8.841,21,9.2,21,12S20.988,15.159,20.938,16.244Z"></path><path d="M17.581,5.467a.953.953,0,1,0,.952.952A.954.954,0,0,0,17.581,5.467Z"></path><path d="M12,7.073A4.927,4.927,0,1,0,16.927,12,4.932,4.932,0,0,0,12,7.073Zm0,8.854A3.927,3.927,0,1,1,15.927,12,3.932,3.932,0,0,1,12,15.927Z"></path></svg>
        `;
    case "Threads":
      return `
    <svg class="" viewBox="0 0 24 24" fill="currentColor"><path d="M16.7434 11.2696C16.6573 11.2283 16.5698 11.1886 16.4811 11.1505C16.3268 8.30606 14.7725 6.6776 12.1627 6.66094C12.1509 6.66086 12.1391 6.66086 12.1273 6.66086C10.5663 6.66086 9.26803 7.32718 8.46896 8.53966L9.90427 9.52425C10.5012 8.61857 11.438 8.4255 12.128 8.4255C12.1359 8.4255 12.1439 8.4255 12.1518 8.42557C13.0111 8.43105 13.6596 8.6809 14.0793 9.16813C14.3847 9.52284 14.589 10.013 14.6901 10.6316C13.9282 10.5022 13.1043 10.4623 12.2234 10.5128C9.74215 10.6558 8.14699 12.1029 8.25412 14.1137C8.30849 15.1337 8.81663 16.0112 9.68489 16.5845C10.419 17.0691 11.3645 17.306 12.3471 17.2524C13.6448 17.1813 14.6628 16.6861 15.373 15.7808C15.9124 15.0933 16.2535 14.2024 16.4042 13.0798C17.0226 13.453 17.4809 13.9442 17.7341 14.5346C18.1645 15.5382 18.1896 17.1875 16.8439 18.5321C15.6648 19.71 14.2475 20.2196 12.1056 20.2353C9.7296 20.2177 7.93268 19.4557 6.76438 17.9705C5.67036 16.5798 5.10497 14.571 5.08387 12C5.10497 9.42894 5.67036 7.42017 6.76438 6.02945C7.93268 4.54426 9.72957 3.78229 12.1055 3.76464C14.4987 3.78243 16.327 4.54806 17.54 6.04042C18.1348 6.77225 18.5832 7.69258 18.8789 8.76566L20.5608 8.3169C20.2025 6.99606 19.6386 5.85789 18.8714 4.91396C17.3162 3.0007 15.0419 2.02033 12.1114 2H12.0997C9.17516 2.02026 6.92626 3.00435 5.41545 4.92493C4.07103 6.634 3.37754 9.01205 3.35424 11.993L3.35416 12L3.35424 12.007C3.37754 14.9879 4.07103 17.366 5.41545 19.0751C6.92626 20.9956 9.17516 21.9798 12.0997 22H12.1114C14.7115 21.982 16.5442 21.3013 18.054 19.7928C20.0293 17.8194 19.9698 15.3457 19.3187 13.8272C18.8517 12.7382 17.9611 11.8538 16.7434 11.2696ZM12.2542 15.4903C11.1667 15.5516 10.0369 15.0634 9.98121 14.0179C9.93991 13.2427 10.5329 12.3777 12.321 12.2747C12.5257 12.2629 12.7267 12.2571 12.9241 12.2571C13.5735 12.2571 14.1811 12.3202 14.7335 12.4409C14.5275 15.0141 13.319 15.4319 12.2542 15.4903Z"></path></svg>
        `;
    case "Whatsapp":
      return `
    <svg class="" viewBox="0 0 24 24" fill="currentColor"><g><path d="M18.8,14.634a1.361,1.361,0,0,0-.645-.477l-.165-.081c-.018-.009-1.845-.917-2.208-1.049a.983.983,0,0,0-1.3.349c-.177.266-.721.914-.918,1.2a.409.409,0,0,1-.125-.049c-.058-.028-.137-.063-.233-.105a7.636,7.636,0,0,1-2.2-1.387,12.737,12.737,0,0,1-1.683-2.008.766.766,0,0,1,.086-.093c.1-.1.223-.248.342-.392l.159-.191a2.247,2.247,0,0,0,.338-.538l.044-.09a1.064,1.064,0,0,0-.026-1c-.057-.116-.35-.835-.616-1.481L9.31,6.408a1.123,1.123,0,0,0-1.146-.849c-.192-.01-.412-.012-.63-.012a1.724,1.724,0,0,0-1.269.62A3.989,3.989,0,0,0,5.087,9.074a6.653,6.653,0,0,0,1.435,3.631,14.178,14.178,0,0,0,5.546,4.857,18.659,18.659,0,0,0,1.846.682,4.476,4.476,0,0,0,1.373.208,5.741,5.741,0,0,0,.84-.065,3.729,3.729,0,0,0,2.526-1.826,2.85,2.85,0,0,0,.142-1.927Zm-1.084,1.59A2.852,2.852,0,0,1,15.978,17.4a3.837,3.837,0,0,1-1.762-.107,17.989,17.989,0,0,1-1.752-.647,13.168,13.168,0,0,1-5.122-4.512l-.071-.1A5.548,5.548,0,0,1,6.087,9.074a3.06,3.06,0,0,1,.974-2.295.654.654,0,0,1,.473-.232c.2,0,.4,0,.579.011h0l.133-.01a.685.685,0,0,1,.139.244l.341.827c.279.68.592,1.438.647,1.548.04.079.033.091.026.1l-.048.1a1.323,1.323,0,0,1-.2.327L8.98,9.9c-.1.117-.19.234-.275.318a.957.957,0,0,0-.217,1.249,9.938,9.938,0,0,0,1.854,2.307,8.645,8.645,0,0,0,2.464,1.558c.076.034.138.06.184.083a.969.969,0,0,0,1.282-.2,9.857,9.857,0,0,0,.993-1.306,1,1,0,0,1,.17.051c.229.084,1.6.75,2.1,1l.177.087c.067.032.144.069.2.1A3.126,3.126,0,0,1,17.711,16.224Z"></path><path d="M12,1A11,11,0,0,0,2.792,18.014l-.974,3.711a.5.5,0,0,0,.61.61l3.779-.991A11,11,0,1,0,12,1Zm0,21a9.961,9.961,0,0,1-5.437-1.614.493.493,0,0,0-.4-.064L3,21.151l.814-3.1a.5.5,0,0,0-.071-.41A10,10,0,1,1,12,22Z"></path></g></svg>
        `;
    case "Tiktok":
      return `
    <svg class="" viewBox="0 0 24 24" fill="currentColor"><path d="M9.37,23.5a7.468,7.468,0,0,1,0-14.936.537.537,0,0,1,.538.5v3.8a.542.542,0,0,1-.5.5,2.671,2.671,0,1,0,2.645,2.669.432.432,0,0,1,0-.05V1a.5.5,0,0,1,.5-.5h3.787a.5.5,0,0,1,.5.5A4.759,4.759,0,0,0,21.59,5.76a.5.5,0,0,1,.5.5L22.1,10a.533.533,0,0,1-.519.515,9.427,9.427,0,0,1-4.741-1.268v6.789A7.476,7.476,0,0,1,9.37,23.5ZM8.908,9.585a6.466,6.466,0,1,0,6.93,6.447V8.326a.5.5,0,0,1,.791-.407A8.441,8.441,0,0,0,21.1,9.5l-.006-2.76A5.761,5.761,0,0,1,15.859,1.5H13.051V16.032a.458.458,0,0,1,0,.053,3.672,3.672,0,1,1-4.14-3.695Z"></path></svg>
        `;
    case "Facebook":
      return `
    <svg class="" viewBox="0 0 24 24" fill="currentColor"><path d="M23,12A11,11,0,1,0,10.279,22.865h0a11.08,11.08,0,0,0,3.436,0h0A10.952,10.952,0,0,0,23,12ZM10.859,21.935v-6.9a.5.5,0,0,0-.5-.5H8.193V12.5h2.166a.5.5,0,0,0,.5-.5V9.686c0-2.278,1.264-3.585,3.459-3.585a15.392,15.392,0,0,1,1.858.137V7.89h-.824l-.019,0a2,2,0,0,0-2.181,1.735,1.8,1.8,0,0,0-.011.4V12a.5.5,0,0,0,.5.5H15.97l-.312,2.035H13.641a.5.5,0,0,0-.5.5v6.9A10.124,10.124,0,0,1,10.859,21.935Zm3.282-.166V15.535h1.946a.5.5,0,0,0,.5-.425l.465-3.035a.5.5,0,0,0-.494-.575H14.141V10.016a1.267,1.267,0,0,1,.308-.821,1.218,1.218,0,0,1,.9-.3h1.324a.5.5,0,0,0,.5-.5V5.806a.5.5,0,0,0-.42-.494A16.661,16.661,0,0,0,14.325,5.1c-2.754,0-4.466,1.757-4.466,4.585V11.5H7.693a.5.5,0,0,0-.5.5v3.035a.5.5,0,0,0,.5.5H9.859v6.234a10,10,0,1,1,4.282,0Z"></path></svg>
        `;
    case "Email":
      return `
    <svg class="" viewBox="0 0 24 24" fill="currentColor"><path d="M18.821,20.5H5.179A3.683,3.683,0,0,1,1.5,16.821V7.179A3.683,3.683,0,0,1,5.179,3.5H18.821A3.683,3.683,0,0,1,22.5,7.179v9.642A3.683,3.683,0,0,1,18.821,20.5ZM5.179,4.5A2.682,2.682,0,0,0,2.5,7.179v9.642A2.682,2.682,0,0,0,5.179,19.5H18.821A2.682,2.682,0,0,0,21.5,16.821V7.179A2.682,2.682,0,0,0,18.821,4.5Z"></path><path d="M12,14.209a.5.5,0,0,1-.346-.138L4.286,7.028a.5.5,0,0,1,.691-.723L12,13.018l7.023-6.713a.5.5,0,1,1,.691.723l-7.368,7.043A.5.5,0,0,1,12,14.209Z"></path><path d="M4.7,17.833a.5.5,0,0,1-.347-.86l5.54-5.31a.5.5,0,0,1,.692.722L5.048,17.694A.5.5,0,0,1,4.7,17.833Z"></path><path d="M19.3,17.832a.5.5,0,0,1-.346-.139l-5.538-5.308a.5.5,0,0,1,.692-.722l5.538,5.308a.5.5,0,0,1-.346.861Z"></path></svg>
        `;
    case "Discord":
      return `
          <svg class="" viewBox="0 0 24 24" fill="currentColor"><path d="M6.613 6.27a.5.5 0 1 0 .274.96l-.274-.96Zm10.5.96a.5.5 0 0 0 .274-.96l-.274.96ZM6.304 16.77a.5.5 0 0 0-.275.96l.275-.96Zm11.667.96a.5.5 0 1 0-.275-.96l.275.96Zm-1.388.103a.5.5 0 1 0-1 0h1Zm5.917 0 .447.224a.495.495 0 0 0 .017-.038l-.464-.186ZM20.75 4.417l.471-.167a.5.5 0 0 0-.185-.244l-.286.41Zm-5.25-1.75.053-.497a.5.5 0 0 0-.517.311l.464.186Zm-1.63 2.73a.5.5 0 1 0 .928.372l-.929-.371ZM8.416 17.834a.5.5 0 0 0-1 0h1Zm-6.526 0-.467.178c.004.013.01.025.015.037l.452-.215ZM3.557 4.417l-.295-.404a.5.5 0 0 0-.18.245l.475.159ZM8.5 2.667l.464-.186a.5.5 0 0 0-.519-.311l.055.497Zm.702 3.102a.5.5 0 1 0 .929-.371l-.929.371ZM9.167 12a.667.667 0 0 1-.667.667v1c.92 0 1.667-.746 1.667-1.667h-1Zm-.667.667A.667.667 0 0 1 7.833 12h-1c0 .92.747 1.667 1.667 1.667v-1ZM7.833 12c0-.368.299-.667.667-.667v-1c-.92 0-1.667.747-1.667 1.667h1Zm.667-.667c.368 0 .667.299.667.667h1c0-.92-.747-1.667-1.667-1.667v1Zm7.667.667a.667.667 0 0 1-.667.667v1c.92 0 1.667-.746 1.667-1.667h-1Zm-.667.667a.667.667 0 0 1-.667-.667h-1c0 .92.746 1.667 1.667 1.667v-1ZM14.833 12c0-.368.299-.667.667-.667v-1c-.92 0-1.667.747-1.667 1.667h1Zm.667-.667c.368 0 .667.299.667.667h1c0-.92-.747-1.667-1.667-1.667v1ZM6.887 7.231c2.018-.577 3.573-.856 5.113-.856 1.54 0 3.095.28 5.113.856l.274-.962c-2.065-.59-3.719-.894-5.387-.894-1.668 0-3.322.304-5.387.894l.274.962Zm-.858 10.5c4.173 1.192 7.769 1.192 11.942 0l-.275-.962c-3.994 1.141-7.398 1.141-11.392 0l-.275.962Zm9.554.102c0 .398.144.84.327 1.246.189.416.442.844.712 1.229.268.383.562.738.837 1.003.137.131.28.251.421.341.127.081.318.181.537.181v-1c.073 0 .082.028 0-.024a1.822 1.822 0 0 1-.264-.219 6.027 6.027 0 0 1-.712-.856 6.848 6.848 0 0 1-.62-1.068c-.163-.36-.238-.647-.238-.833h-1Zm2.834 4c1.061 0 2.006-.585 2.753-1.307.755-.728 1.369-1.653 1.777-2.469l-.894-.447c-.37.74-.922 1.564-1.578 2.197-.662.64-1.37 1.026-2.058 1.026v1Zm4.547-3.814c.435-1.087.57-2.887.325-5.197-.248-2.329-.89-5.235-2.068-8.572l-.942.333c1.155 3.275 1.777 6.105 2.016 8.345.24 2.26.084 3.862-.26 4.72l.93.371ZM21.036 4.006c-1.8-1.254-3.699-1.646-5.483-1.836l-.106.994c1.716.183 3.418.549 5.017 1.663l.572-.82Zm-6-1.525-1.167 2.917.929.371 1.166-2.917-.928-.37Zm-7.62 15.352c0 .196-.07.49-.218.85-.144.35-.344.725-.563 1.07-.22.345-.451.645-.649.852-.1.105-.18.174-.238.213-.073.05-.054.015.031.015v1c.225 0 .415-.108.534-.19.135-.092.27-.214.396-.347.255-.267.524-.623.769-1.006.245-.385.475-.813.645-1.228.167-.405.294-.84.294-1.229h-1Zm-1.637 3c-.638 0-1.304-.377-1.935-1.017-.623-.632-1.149-1.457-1.502-2.198l-.903.43c.388.815.973 1.74 1.693 2.47.71.72 1.618 1.315 2.647 1.315v-1Zm-3.42-3.178c-.33-.861-.478-2.469-.249-4.73.228-2.242.82-5.073 1.92-8.349l-.947-.318c-1.12 3.335-1.732 6.24-1.967 8.567-.234 2.308-.105 4.104.308 5.186l.934-.356ZM3.851 4.82c1.52-1.111 3.049-1.474 4.703-1.656l-.11-.994c-1.74.191-3.465.587-5.183 1.843l.59.807Zm4.184-1.968L9.202 5.77l.929-.371L8.964 2.48l-.928.371Z"></path></svg>
        `;
    case "Youtube":
      return `
    <svg class="" viewBox="0 0 24 24" fill="currentColor"><path d="M12,20.55c-.3,0-7.279-.006-9.115-.5A3.375,3.375,0,0,1,.5,17.665,29.809,29.809,0,0,1,0,12,29.824,29.824,0,0,1,.5,6.334,3.375,3.375,0,0,1,2.885,3.948c1.836-.492,8.819-.5,9.115-.5s7.279.006,9.115.5A3.384,3.384,0,0,1,23.5,6.334,29.97,29.97,0,0,1,24,12a29.97,29.97,0,0,1-.5,5.666,3.384,3.384,0,0,1-2.388,2.386C19.279,20.544,12.3,20.55,12,20.55Zm0-16.1c-.072,0-7.146.006-8.857.464A2.377,2.377,0,0,0,1.464,6.593,29.566,29.566,0,0,0,1,12a29.566,29.566,0,0,0,.464,5.407,2.377,2.377,0,0,0,1.679,1.679c1.711.458,8.785.464,8.857.464s7.146-.006,8.857-.464a2.377,2.377,0,0,0,1.679-1.679A29.66,29.66,0,0,0,23,12a29.66,29.66,0,0,0-.464-5.407h0a2.377,2.377,0,0,0-1.679-1.679C19.146,4.456,12.071,4.45,12,4.45ZM9.7,15.95a.5.5,0,0,1-.5-.5V8.55a.5.5,0,0,1,.75-.433l5.975,3.45a.5.5,0,0,1,0,.866L9.95,15.883A.5.5,0,0,1,9.7,15.95Zm.5-6.534v5.168L14.675,12Z"></path></svg>
        `;
    case "Snapchat":
      return `
    <svg class="" viewBox="0 0 24 24" fill="currentColor"><path d="M12,22.5a6.309,6.309,0,0,1-3.574-1.315,2.457,2.457,0,0,0-2.3-.53,1.679,1.679,0,0,1-1.294-.018,1.522,1.522,0,0,1-.615-1.051c-.031-.125-.059-.159-.621-.284a7.775,7.775,0,0,1-.9-.245,1.512,1.512,0,0,1-1.19-1.5.968.968,0,0,1,.729-.907A5.19,5.19,0,0,0,5.885,12.8a11.574,11.574,0,0,1-1.808-1.067A1.816,1.816,0,0,1,6.138,8.741C6.11,8.357,6.09,7.9,6.091,7.375a5.909,5.909,0,0,1,11.818,0c0,.522-.018.982-.047,1.366a1.816,1.816,0,0,1,2.062,2.987A11.542,11.542,0,0,1,18.115,12.8a5.19,5.19,0,0,0,3.652,3.856.991.991,0,0,1,.732.939,1.524,1.524,0,0,1-1.193,1.466,7.775,7.775,0,0,1-.9.245c-.562.125-.59.159-.621.285a1.522,1.522,0,0,1-.615,1.05,1.679,1.679,0,0,1-1.294.018,2.458,2.458,0,0,0-2.3.529A6.3,6.3,0,0,1,12,22.5ZM6.789,19.552a3.914,3.914,0,0,1,2.19.8A5.367,5.367,0,0,0,12,21.5a5.367,5.367,0,0,0,3.021-1.149,3.464,3.464,0,0,1,3.132-.656c.405.118.515.075.516.075a1.477,1.477,0,0,0,.141-.423c.187-.757.791-.891,1.375-1.021a7.09,7.09,0,0,0,.793-.213c.508-.177.522-.438.522-.467l0-.021a6.226,6.226,0,0,1-4.393-4.748.822.822,0,0,1,.464-.93,10.283,10.283,0,0,0,1.756-1.02.817.817,0,0,0,.319-.537.807.807,0,0,0-.155-.6.817.817,0,0,0-1.141-.166,8.194,8.194,0,0,1-.852.536.5.5,0,0,1-.737-.512,15.116,15.116,0,0,0,.15-2.267,4.909,4.909,0,0,0-9.818,0,15.246,15.246,0,0,0,.15,2.267.5.5,0,0,1-.737.512,8.247,8.247,0,0,1-.851-.535.816.816,0,0,0-1.142.165.813.813,0,0,0,.165,1.141,10.206,10.206,0,0,0,1.755,1.02.823.823,0,0,1,.464.931,6.208,6.208,0,0,1-4.4,4.74c.008.057.022.318.53.5a7.09,7.09,0,0,0,.793.213c.584.13,1.188.264,1.375,1.021a1.329,1.329,0,0,0,.147.427h0a1.12,1.12,0,0,0,.51-.079A3.335,3.335,0,0,1,6.789,19.552Z"></path></svg>
        `;
    case "Amazon":
      return `
          <svg class="" viewBox="0 0 24 24" fill="currentColor"><path d="M11.058,15.759a2.671,2.671,0,0,0,2.3-1.638,6.812,6.812,0,0,0,.631-3.608l0-.452a.5.5,0,0,0-.5-.5c-3.145,0-4.738,1.2-4.738,3.571A2.349,2.349,0,0,0,11.058,15.759Zm1.934-5.187a5.877,5.877,0,0,1-.5,3.052l0,.008c-.437.79-.864,1.127-1.431,1.127-1.18,0-1.307-1.138-1.307-1.627C9.751,12,10.132,10.7,12.992,10.572Z"></path><path d="M7.866,8.278a5.565,5.565,0,0,0-3.454,5.316c0,3.132,1.946,5.156,4.959,5.156a6.11,6.11,0,0,0,4.8-1.911,6.349,6.349,0,0,0,1.512,1.655A1.028,1.028,0,0,0,17,18.341c.579-.519,1.657-1.437,2.309-1.986a1.119,1.119,0,0,0,.073-1.4.4.4,0,0,0-.023-.034c-.105-.145-.216-.281-.325-.416a2.642,2.642,0,0,1-.784-1.918V7.813c0-2.028,0-3.944-1.539-5.438A6.945,6.945,0,0,0,12.043.75c-1.858,0-6.27.483-7.118,4.948a1.059,1.059,0,0,0,.9,1.247l2.966.323a1.175,1.175,0,0,0,.955-.983A1.758,1.758,0,0,1,11.64,4.9a1.2,1.2,0,0,1,.9.3,3.175,3.175,0,0,1,.454,2.066l-.381.042A16.647,16.647,0,0,0,7.866,8.278Zm5.68-.069a.5.5,0,0,0,.443-.5V7.52a4.218,4.218,0,0,0-.745-3.028,2.174,2.174,0,0,0-1.6-.591A2.729,2.729,0,0,0,8.763,6.074a.5.5,0,0,1-.081.185l-2.693-.3c-.05-.011-.087-.038-.084-.062.711-3.745,4.529-4.149,6.138-4.149a5.948,5.948,0,0,1,3.969,1.344c1.235,1.2,1.235,2.829,1.235,4.719V12.59a3.623,3.623,0,0,0,1.007,2.547c.094.117.19.235.282.359.049.086.06.153.095.123-.626.525-1.717,1.453-2.3,1.976-.042.037-.083.058-.071.08a5.608,5.608,0,0,1-1.346-1.528l-.252-.37a.5.5,0,0,0-.369-.213.464.464,0,0,0-.4.149A5.379,5.379,0,0,1,9.371,17.75c-2.479,0-3.959-1.554-3.959-4.156A4.547,4.547,0,0,1,8.263,9.2,15.963,15.963,0,0,1,12.714,8.3Z"></path><path d="M23.794,18.381c-.523-.595-3.211-.788-4.748.31a.5.5,0,1,0,.582.814A4.6,4.6,0,0,1,23,19.02a4.553,4.553,0,0,1-1.331,3.162.5.5,0,1,0,.646.762C23.731,21.742,24.34,19.1,23.794,18.381Z"></path><path d="M21.859,20.024a.5.5,0,0,0-.678-.2c-.187.1-.374.2-.558.292A18.948,18.948,0,0,1,.816,18.566a.5.5,0,0,0-.634.774,20.327,20.327,0,0,0,11.832,3.892,19.953,19.953,0,0,0,9.059-2.227c.193-.1.388-.2.583-.3A.5.5,0,0,0,21.859,20.024Z"></path></svg>
        `;
    case "Etsy":
      return `
    <svg class="" viewBox="0 0 24 24" fill="currentColor"><path d="M19.793,3s-.136,1.454-.3,4.713H18.157l-.119-.471C17.6,5.513,17.01,4.594,15.93,4.594H9.74c-.46,0-.486.161-.486.434l0,6.592c.009,0,.749.029,1.681.029,1.565,0,3.672-.081,3.777-.515l.379-1.568h1.163L16.1,12.283,16.256,15H15.093l-.379-1.568c-.105-.433-2.212-.515-3.777-.515-.931,0-1.669.029-1.68.03l0,4.92c0,1.163.649,1.7,2,1.7h4.027c1.352,0,2.568-.135,3.406-2.027l.547-1.255h1.347C20.4,17.8,20.1,20.3,20.037,21c0,0-3.7-.135-5.3-.135H7.658L3.413,21V19.865l1.433-.27c1-.189,1.325-.488,1.325-1.3,0,0,.08-1.729.08-6.189s-.08-6.216-.08-6.216c-.083-.919-.325-1.134-1.325-1.324L3.413,4.3V3.161L7.658,3.3h7.919c1.568,0,4.216-.3,4.216-.3m.794,13.287h0M19.793,2a1.032,1.032,0,0,0-.111.006c-.026,0-2.612.29-4.1.29H7.658L3.445,2.162H3.413a1,1,0,0,0-1,1V4.3a1,1,0,0,0,.815.983l1.433.27a2.752,2.752,0,0,1,.467.117,2.071,2.071,0,0,1,.047.314h0s.076,1.783.076,6.126c0,4.373-.079,6.126-.079,6.143a1.794,1.794,0,0,1-.013.224,2.12,2.12,0,0,1-.5.138l-1.432.27a1,1,0,0,0-.815.982V21a1,1,0,0,0,1,1h.032l4.245-.135h7.048c1.56,0,5.226.133,5.263.134h.037a1,1,0,0,0,1-.918c.056-.681.349-3.121.536-4.6a1.059,1.059,0,0,0,.019-.2,1,1,0,0,0-1-1H19.24a1,1,0,0,0-.917.6l-.547,1.255c-.572,1.292-1.2,1.427-2.489,1.427H11.26c-1,0-1-.28-1-.7l0-3.945c.207,0,.437,0,.68,0a16.364,16.364,0,0,1,2.907.171l.277,1.147a1,1,0,0,0,.972.765h1.163a1,1,0,0,0,1-1.059L17.1,12.283l.157-2.658a1,1,0,0,0-1-1.059H15.093a1,1,0,0,0-.972.765l-.277,1.148a16.523,16.523,0,0,1-2.907.17c-.244,0-.474,0-.681,0V5.6l5.675,0c.181,0,.664,0,1.138,1.892l.12.474a1,1,0,0,0,.969.753h1.332a1,1,0,0,0,1-.948c.165-3.193.3-4.658.3-4.672a1,1,0,0,0-1-1.093Zm0,2h0Z"></path></svg>
        `;
    case "Linkedin":
      return `
    <svg class="" viewBox="0 0 24 24" fill="currentColor"><path d="M4.425,1.671A2.738,2.738,0,0,0,1.5,4.4,2.71,2.71,0,0,0,4.369,7.128H4.4a2.734,2.734,0,1,0,.028-5.457ZM4.4,6.128H4.369a1.736,1.736,0,1,1,.056-3.457A1.737,1.737,0,1,1,4.4,6.128Z"></path><path d="M6.541,8.431H2.253a.5.5,0,0,0-.5.5v12.9a.5.5,0,0,0,.5.5H6.541a.5.5,0,0,0,.5-.5V8.931A.5.5,0,0,0,6.541,8.431Zm-.5,12.9H2.753V9.431H6.041Z"></path><path d="M17.064,8.128A4.691,4.691,0,0,0,13.7,9.362V8.931a.5.5,0,0,0-.5-.5H8.914a.5.5,0,0,0-.5.523c.053,1.183,0,12.756,0,12.873a.5.5,0,0,0,.5.5H13.2a.5.5,0,0,0,.5-.5v-7.2a2.749,2.749,0,0,1,.1-.86,1.869,1.869,0,0,1,1.737-1.254c.413,0,1.671,0,1.671,2.417v6.9a.5.5,0,0,0,.5.5H22a.5.5,0,0,0,.5-.5v-7.4C22.5,10.485,20.467,8.128,17.064,8.128Zm4.436,13.2H18.213v-6.4c0-2.973-1.673-3.417-2.671-3.417a2.83,2.83,0,0,0-2.664,1.878,3.253,3.253,0,0,0-.177,1.236v6.7H9.416c.009-2.058.04-9.654.009-11.9H12.7v1.328a.5.5,0,0,0,.92.272,3.769,3.769,0,0,1,3.443-1.9c2.819,0,4.436,1.934,4.436,5.305Z"></path></svg>
        `;
    case "Patreon":
      return `
    <svg class="" viewBox="0 0 24 24" fill="currentColor"><path d="M15.011,17A7.5,7.5,0,1,1,22.5,9.51,7.509,7.509,0,0,1,15.011,17Zm0-14A6.5,6.5,0,1,0,21.5,9.51,6.518,6.518,0,0,0,15.011,3Z"></path><path d="M5.5,22H2a.5.5,0,0,1-.5-.5V2.5A.5.5,0,0,1,2,2H5.5a.5.5,0,0,1,.5.5v19A.5.5,0,0,1,5.5,22Zm-3-1H5V3H2.5Z"></path></svg>
        `;
    case "Pinterest":
      return `
    <svg class="" viewBox="0 0 24 24" fill="currentColor"><path d="M7.615,23a.522.522,0,0,1-.11-.012.5.5,0,0,1-.386-.429l-.166-1.4A12.6,12.6,0,0,1,7,17.381c.205-.882,1.223-5.2,1.432-6.091a5.332,5.332,0,0,1-.348-1.907c0-1.992,1.219-3.552,2.776-3.552A2.07,2.07,0,0,1,12.956,8.1a10.218,10.218,0,0,1-.6,2.723c-.161.523-.328,1.06-.454,1.584a1.3,1.3,0,0,0,.225,1.133,1.441,1.441,0,0,0,1.15.525c2,0,3.452-2.209,3.452-5.252A4.482,4.482,0,0,0,11.969,4.2,4.957,4.957,0,0,0,6.79,9.155a4.7,4.7,0,0,0,.457,1.963.873.873,0,0,1,.063.584l-.428,1.816a.79.79,0,0,1-.515.571.813.813,0,0,1-.776-.139A6.575,6.575,0,0,1,3.472,9.1c0-3.9,2.756-8.1,8.807-8.1,4.7,0,8.249,3.337,8.249,7.762,0,4.743-3.017,8.321-7.018,8.321a4.281,4.281,0,0,1-2.635-.9l-.543,2.069a14.305,14.305,0,0,1-1.62,3.435l-.673,1.079A.5.5,0,0,1,7.615,23ZM10.862,6.831c-.995,0-1.776,1.121-1.776,2.552a4.149,4.149,0,0,0,.322,1.642.514.514,0,0,1,.037.334s-1.24,5.264-1.468,6.246a11.938,11.938,0,0,0-.034,3.422A12.932,12.932,0,0,0,9.364,18l.787-2.993a.5.5,0,0,1,.43-.37.51.51,0,0,1,.5.269,2.849,2.849,0,0,0,2.43,1.177c3.431,0,6.018-3.148,6.018-7.321C19.528,4.907,16.412,2,12.279,2,6.916,2,4.472,5.682,4.472,9.1a5.769,5.769,0,0,0,1.514,3.859l.351-1.492A5.224,5.224,0,0,1,5.79,9.155,5.937,5.937,0,0,1,11.969,3.2a5.438,5.438,0,0,1,5.755,5.61c0,3.623-1.872,6.252-4.452,6.252a2.44,2.44,0,0,1-1.936-.9,2.283,2.283,0,0,1-.412-1.987c.132-.545.3-1.1.472-1.646a9.754,9.754,0,0,0,.56-2.427A1.11,1.11,0,0,0,10.862,6.831Z"></path></svg>
        `;
    case "Spotify":
      return `
    <svg class="" viewBox="0 0 24 24" fill="currentColor"><path d="M18,10.561a.494.494,0,0,1-.245-.065,15.2,15.2,0,0,0-10.95-1.55.5.5,0,0,1-.232-.973A16.2,16.2,0,0,1,18.25,9.626a.5.5,0,0,1-.247.935Z"></path><path d="M16.646,13.632a.5.5,0,0,1-.249-.066,12.459,12.459,0,0,0-9.121-1.292.5.5,0,1,1-.237-.971A13.458,13.458,0,0,1,16.9,12.7a.5.5,0,0,1-.25.933Z"></path><path d="M15.312,16.583a.5.5,0,0,1-.251-.068,9.777,9.777,0,0,0-7.295-1.033.5.5,0,0,1-.245-.97,10.768,10.768,0,0,1,8.043,1.139.5.5,0,0,1-.252.932Z"></path><path d="M12,23A11,11,0,1,1,23,12,11.013,11.013,0,0,1,12,23ZM12,2A10,10,0,1,0,22,12,10.011,10.011,0,0,0,12,2Z"></path></svg>
        `;
    case "Soundcloud":
      return `
    <svg class="" viewBox="0 0 24 24" fill="currentColor"><path d="M19.119,19.071H9.262a.5.5,0,0,1,0-1h9.857a3.881,3.881,0,1,0,0-7.761,1.084,1.084,0,0,0-.161.016l-.165.019a.482.482,0,0,1-.515-.334A6.055,6.055,0,0,0,9.513,6.748a.5.5,0,0,1-.5-.866A7.056,7.056,0,0,1,19.082,9.31h.037a4.881,4.881,0,1,1,0,9.761Z"></path><path d="M.5,17.429a.5.5,0,0,1-.5-.5V10.357a.5.5,0,0,1,1,0v6.572A.5.5,0,0,1,.5,17.429Z"></path><path d="M3.238,19.071a.5.5,0,0,1-.5-.5V7.619a.5.5,0,0,1,1,0V18.571A.5.5,0,0,1,3.238,19.071Z"></path><path d="M5.977,19.071a.5.5,0,0,1-.5-.5V8.714a.5.5,0,0,1,1,0v9.857A.5.5,0,0,1,5.977,19.071Z"></path></svg>
        `;
    case "Telegram":
      return `
    <svg class="" viewBox="0 0 24 24" fill="currentColor"><path d="M11 0a11 11 0 1011 11A11.012 11.012 0 0011 0zm0 21a10 10 0 1110-10 10.011 10.011 0 01-10 10zm4.464-15.366a2.405 2.405 0 00-.821.155c-.114.047-2.402.985-3.101 1.29L4.489 9.984c-.827.323-1.284.801-1.342 1.416a1.414 1.414 0 00.677 1.298l.097.06.108.038c.044.015 1.091.373 2.01.649a3.527 3.527 0 001.01.156 2.31 2.31 0 001.203-.296l.07-.047-.001.044a1.617 1.617 0 00.602 1.219c.67.566 2.133 1.523 2.836 1.982.15.099.257.168.296.195a3.755 3.755 0 001.939.752 1.71 1.71 0 00.39-.044 1.859 1.859 0 001.34-1.665l.755-4.736.198-1.295c.193-1.254.304-1.98.327-2.31a1.626 1.626 0 00-.388-1.289 1.534 1.534 0 00-1.152-.477zm.027 5.214l-.759 4.765s-.06.698-.576.82a.707.707 0 01-.162.017 2.892 2.892 0 01-1.356-.564c-.121-.091-2.276-1.457-3.065-2.124a.575.575 0 01.03-.972 115.996 115.996 0 003.187-3.034c.236-.236.472-.676.196-.676a2.37 2.37 0 00-.985.494l-4.279 2.883a1.353 1.353 0 01-.673.144 2.514 2.514 0 01-.723-.114c-.91-.273-1.972-.637-1.972-.637s-.729-.456.515-.941l7.071-2.913c.698-.304 3.065-1.275 3.065-1.275a1.44 1.44 0 01.46-.087c.286 0 .592.128.542.694-.03.425-.273 1.912-.516 3.52z"></path></svg>
        `;
    case "Twitch":
      return `
    <svg class="" viewBox="0 0 24 24" fill="currentColor"><path d="M7.285,23.5a.493.493,0,0,1-.191-.038A.5.5,0,0,1,6.785,23V19.571H2.572a.5.5,0,0,1-.5-.5V4.929a.5.5,0,0,1,.146-.354L6.147.646A.5.5,0,0,1,6.5.5H21.429a.5.5,0,0,1,.5.5V12a.5.5,0,0,1-.147.354L14.71,19.425a.5.5,0,0,1-.353.146H11.421L7.639,23.354A.5.5,0,0,1,7.285,23.5ZM3.072,18.571H7.285a.5.5,0,0,1,.5.5v2.722l3.076-3.075a.5.5,0,0,1,.353-.147H14.15l6.779-6.778V1.5H6.707L3.072,5.136Z"></path><path d="M10.822,17.607a.494.494,0,0,1-.192-.038.5.5,0,0,1-.308-.462v-2.25H7.285a.5.5,0,0,1-.5-.5V3.571a.5.5,0,0,1,.5-.5H18.857a.5.5,0,0,1,.5.5v7.643a.5.5,0,0,1-.147.354L16.068,14.71a.5.5,0,0,1-.354.147H13.779l-2.6,2.6A.5.5,0,0,1,10.822,17.607Zm-3.037-3.75h3.037a.5.5,0,0,1,.5.5V15.9l1.9-1.9a.5.5,0,0,1,.354-.146h1.935l2.85-2.85V4.071H7.785Z"></path><path d="M15.857,10.386a.5.5,0,0,1-.5-.5V6.072a.5.5,0,1,1,1,0V9.886A.5.5,0,0,1,15.857,10.386Z"></path><path d="M12.357,10.386a.5.5,0,0,1-.5-.5V6.072a.5.5,0,1,1,1,0V9.886A.5.5,0,0,1,12.357,10.386Z"></path></svg>
        `;
    // Handle other categories here if needed
    default:
      return LinkItem; // Replace 'DefaultComponent' with a fallback component name
  }
}

watch(
  () => store.state.guestpagedata.data,
  async () => setPageData()
);
</script>
